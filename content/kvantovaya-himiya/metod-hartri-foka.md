...
Уравнения записаны для каждой орбитали: такое же уравнение с $\psi_2$ и $\varepsilon_2$, …, с $\psi_n$ и $\varepsilon_n$. Множитель 2 перед кулоновским слагаемым появился потому, что на каждой орбитали два электрона — со спинами $\alpha$ и $\beta$. Обменное слагаемое со знаком минус означает, что два электрона с одинаковыми спинами стараются расположиться подальше друг от друга.

### Оператор Фока

Первая скобка в уравнении — оператор (кинетическая энергия и притяжение к ядру). Интеграл при $2\psi_1(1)$ — тоже оператор: он действует на $\psi_1(1)$ умножением. А вот в обменном слагаемом искомая функция $\psi_1$ стоит внутри интеграла — ее нельзя вынести за знак оператора. Но можно записать упрощенно:

$$
\widehat{F}_1\psi_1 = \varepsilon_1\psi_1, \qquad
\widehat{F}_2\psi_2 = \varepsilon_2\psi_2, \qquad \dots, \qquad
\widehat{F}_n\psi_n = \varepsilon_n\psi_n
$$

$\widehat{F}$ — **оператор Фока** (фокиан). Несет информацию об энергии системы. Мы не можем записать его отдельно, т. к. искомая функция $\psi$ находится внутри оператора.

Поэтому система решается так же, как система Хартри: задаются функции нулевого приближения и подставляются в эти интегралы. Полученный набор сравнивается с предыдущим; если они разные — по новой. Функция орбитали получается в виде таблицы значений.

### Энергия орбитали и полная энергия

$$
\varepsilon_i = H_i + \sum_j \left( 2J_{ij} - K_{ij} \right)
$$

Здесь $H_i$ — остовный интеграл, $J_{ij}$ — кулоновский интеграл (те же, что в методе Хартри), а $K_{ij}$ — новый, **обменный интеграл**, в котором электроны 1 и 2 «поменялись» орбиталями:

$$
K_{ij} = \int \psi_i(2)\,\psi_j(1)\,\frac{e^2}{r_{12}}\,\psi_j(2)\,\psi_i(1)\,d\tau_1\,d\tau_2
$$

Полная энергия атома:

$$
E = 2\sum_i H_i + \sum_i \sum_j \left( 2J_{ij} - K_{ij} \right)
$$

### Волновая функция в методе Хартри — Фока

Полная волновая функция — определитель (детерминант Слейтера), но теперь каждая орбиталь входит в него дважды: со спиновой функцией $\alpha$ и со спиновой функцией $\beta$. Так $n$ орбиталей описывают $2n$ электронов:

$$
\psi = \frac{1}{\sqrt{(2n)!}}
\begin{vmatrix}
\psi_1(1)\alpha(1) & \psi_1(2)\alpha(2) & \dots & \psi_1(2n)\alpha(2n) \\
\psi_1(1)\beta(1) & \psi_1(2)\beta(2) & \dots & \psi_1(2n)\beta(2n) \\
\psi_2(1)\alpha(1) & \psi_2(2)\alpha(2) & \dots & \psi_2(2n)\alpha(2n) \\
\vdots & \vdots & & \vdots \\
\psi_n(1)\beta(1) & \psi_n(2)\beta(2) & \dots & \psi_n(2n)\beta(2n)
\end{vmatrix}
$$
